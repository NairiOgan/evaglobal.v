...
// =====================================================================
// Конструктор
// Получаем все радиокнопки для цвета коврика
const colorBodyRadios = document.querySelectorAll('input[name="colorBody"]');

// Получаем все радиокнопки для цвета канта
...
if (document.querySelector('.colorBodyOutput')) {
  // Инициализируем текстовые значения при загрузке страницы
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (document.querySelector('.product-section-carpet')) {
    ...
    function updateCounter(change) {
      ...
      if (currentValue + change >= 1 && currentValue + change <= 8) {
        ...
        counterInput.setAttribute('value', currentValue);

        if (document.querySelector('._with-side').classList.contains('active')) {
          updateTotalPrice(); // Вызываем функцию для опций с бортом
          console.log(1)
        } if (document.querySelector('._without-side').classList.contains('active')) {
          // Вызываем функцию для опций без борта
  ...
  if (document.querySelector('.product-section-carpet')) {
    ...
    withoutSideBtn.addEventListener('change', function () {
      ...
      carpetSaddle.addEventListener('change', updateTotalPriceWithoutS);
      carpetLogo.addEventListener('change', updateTotalPriceWithoutS);
